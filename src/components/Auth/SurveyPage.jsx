// src/components/Auth/SurveyPage.jsx
...
const SurveyPage = () => {
  ...
  const memberId = searchParams.get('memberId'); // ✅ URL에서 memberId 추출

  const handleSubmit = async () => {
    if (!name || !gender || !handedness || !racket || !level || !gu) {
      alert('모든 항목을 입력해주세요!');
      return;
    }
  ...
  const groupByInitial = (districts) => {
    ...
    const ranges = [
      ['가','나'],['나','다'],['다','라'],['라','마'],['마','바'],['바','사'],['사','아'],
      ['아','자'],['자','차'],['차','카'],['카','타'],['타','파'],['파','하'],['하','힣']
    ];
